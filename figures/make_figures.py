...
def fig_task():
    ...
    sub = (f"blue = agent   ·   red = feature 0 (value {vals[0]:.1f}, {d[0]} steps away)   ·   "
    ...
    fig.suptitle("The agent sees five symbolic channels — no colour, no pixels",
...
def fig_rho_response():
    ...
    for ax, key, title, sub in zip(
        ...
        rho = d["rho"]
        for vals, colour, label in ((d["chose_optimal"], BLUE, "chose optimal"),
        ...
        ax.set_xticks(rho); ax.set_xlim(1.06, -0.06)
        ax.set_ylim(-6, 112); ax.set_yticks([0, 25, 50, 75, 100])
    ...
    fig.suptitle("Trained at ρ=1.0, tested across ρ:  the 5×5 agent tracks colour, not value",
                 y=1.09, fontsize=10.5, color=INK)
...
def fig_margin():
    ...
    for vals, colour, label, dy in ((d["rho1"], BLUE, "ρ = 1.0  (colour agrees with value)", 10),
        ...
        for xi, y in zip(x, vals):
            ax.annotate(f"{y:.0f}%", (xi, y), textcoords="offset points", xytext=(0, dy),
                        ha="center", fontsize=7.5, color=INK2)
    ax.annotate("", xy=(3, d["rho1"][3]), xytext=(3, d["rho0"][3]),
                arrowprops=dict(arrowstyle="<->", color=MUTED, lw=1))
    ax.annotate("27 pts", (3.08, 85), ha="left", fontsize=8, color=INK2)
    ax.annotate("what colour costs\non an easy decision", (3.08, 74), ha="left",
                fontsize=7.5, color=MUTED)
    ax.axhline(50, color=MUTED, lw=0.8, ls=(0, (4, 3)), zorder=1)
    ax.set_xlim(-0.35, 4.35)
    ax.set_xticks(x)
    ax.set_xticklabels([f"{b}\nn={n}" for b, n in zip(d["bins"], d["n"])], fontsize=8)
    ax.set_ylim(-6, 116); ax.set_yticks([0, 25, 50, 75, 100])
    ax.grid(axis="y", zorder=0)
...
def fig_dynamics():
    ...
    for name, colour, label in (("rho100", BLUE, "ρ = 1.0"),
                                ("rho050", AQUA, "ρ = 0.5"),
                                ("rho000", ORANGE, "ρ = 0.0")):
        s_ = arm(proxy, name)
    ...
    ax1.annotate("optimum", (2, M["optimum"]["11x11_fixed_values"]), textcoords="offset points",
                 xytext=(0, 5), fontsize=7.5, color=MUTED)
    ...
    for df, colour, label in ((proxy, BLUE, "trained at ρ = 1.0  (proxy available)"),
    ...
    ax2.annotate("before competence:\ngap is meaningless", (10, -0.135), ha="center",
                 fontsize=7.5, color=MUTED)
    ...
    fig.suptitle("Misgeneralisation appears after competence, and only when a proxy exists",
                 y=0.97, fontsize=10.5, color=INK)
